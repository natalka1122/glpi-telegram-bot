...
async def process_to_enter_title(user_id, text):
    ...
    await bot.send_message(
        user_id,
        "Обращение номер {} создано. Введите /edit для редактирования".format(
            ticket_id
        ),
    )
...
async def edit_ticket(user_id):
    ...
    for current_ticket in list_tickets:
        await bot.send_message(
            user_id,
            "Номер: {} Тема: {}".format(current_ticket["id"], current_ticket["name"]),
        )
...
async def process_to_select_ticket_number(user_id, ticket_id):
    ...
    await bot.send_message(
        user_id,
        "Вот ваш тикет: {}. И какое поле в нем мы хотим поменять? ".format(ticket),
    )
...
async def text_message(message):
    ...
    await bot.send_message(
        user_id, "Ничего не понял. user_id: {} message: {}".format(user_id, text)
    )
