from config import TELEGRAM_TOKEN, LOG_FILENAME, LOG_LEVEL, DB_FILE, SKIT_BASE_URL, STATE_FILE
...
async def create_user_session(user_id):
    ...
    await bot.send_message(user_id, "Вас приветсвует супер-бот СКИТ. Введите /list, что бы получить список тикетов. Введите /add, что бы создать тикет. Введите /edit, что бы отредактировать тикет")
